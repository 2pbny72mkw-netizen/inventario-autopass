...
function renderPoints(){$('panPoints').innerHTML=(current.points||[]).map(p=>`<section class="panPoint"><div class="sectionHead"><div><h3>${esc(p.name)}</h3><small>${p.photos.length} foto(s) · ${esc(p.status)} · Técnico: ${esc(p.technician||'—')}</small></div></div><div class="panGallery">${p.photos.map(ph=>`<figure><a href="${esc(ph.url)}" target="_blank"><img loading="lazy" src="${esc(ph.url)}"></a><figcaption>${esc(ph.uploaded_by)} · ${new Date(ph.created_at).toLocaleString('pt-BR')}${window.PAN_ROLE==='manager'?`<button type="button" class="dangerGhost" data-del="${ph.id}">Excluir</button>`:''}</figcaption></figure>`).join('')||'<p class="muted">Nenhuma foto.</p>'}</div></section>`).join('')||'<p class="muted">Nenhum ponto panorâmico iniciado.</p>';document.querySelectorAll('[data-del]').forEach(b=>b.onclick=async()=>{if(!confirm('Excluir esta foto?'))return;const id=current?.id;const r=await fetch('/api/panoramas/photos/'+b.dataset.del,{method:'DELETE'}),d=await r.json().catch(()=>({}));if(!r.ok){alert(d.error||'Não foi possível excluir a foto.');return}panStatus(`Foto excluída. Status da localidade: ${d.status||'atualizado'}.`,true);await loadPan();if(id&&panData.some(x=>x.id===id))openLoc(id)})}
