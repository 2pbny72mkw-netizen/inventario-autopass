...
// V42.4 — Dashboard do Inventário ATM.
...
function atmBars(id,obj,filterId){const e=document.getElementById(id);if(!e)return;const a=Object.entries(obj||{}).sort((x,y)=>y[1]-x[1]),m=Math.max(1,...a.map(x=>x[1]));e.innerHTML=a.map(([k,v])=>`<button class="barRow v424ClickableBar" data-atm-filter="${filterId}" data-atm-value="${atmEsc(k)}"><span>${atmEsc(k)}</span><i><b style="width:${v/m*100}%"></b></i><strong>${v}</strong></button>`).join('')||'<p class="muted">Sem dados.</p>';e.querySelectorAll('[data-atm-filter]').forEach(b=>b.onclick=()=>{const f=document.getElementById(b.dataset.atmFilter);if(f){f.value=b.dataset.atmValue;loadAtmDashboard(false)}})}
async function loadAtmDashboard(fillOptions=true){const r=await fetch('/api/dashboard/inventory-atm?'+atmParams(),{cache:'no-store'}),d=await r.json();if(!d.ok)return;atmDashData=d;if(fillOptions){atmFill('atmFCompany',d.options.companies,'Todas operadoras');atmFill('atmFLine',d.options.lines,'Todas linhas');atmFill('atmFLocation',d.options.localities,'Todas localidades');atmFill('atmFModel',d.options.models,'Todos modelos');atmFill('atmFContract',d.options.contracts,'Todos contratos');atmFill('atmFOwnership',d.options.ownership,'Todo tipo de posse');atmFill('atmFStatus',d.options.statuses,'Todos status')}document.getElementById('atmKTotal').textContent=d.total;document.getElementById('atmKAllocated').textContent=d.allocated??0;document.getElementById('atmKStock').textContent=d.stock??0;document.getElementById('atmKModels').textContent=Object.keys(d.models||{}).length;const assets=d.assets||[];document.getElementById('atmKContracts').textContent=assets.filter(x=>x.contract&&x.contract!=='SEM CONTRATO').length;document.getElementById('atmKLeasing').textContent=assets.filter(x=>String(x.ownership).toUpperCase()==='LEASING').length;document.getElementById('atmKNoContract').textContent=assets.filter(x=>!x.contract||x.contract==='SEM CONTRATO').length;document.getElementById('atmRowsTag').textContent=d.total+' ATMs';atmBars('atmByOperator',d.operators,'atmFCompany');atmBars('atmByModel',d.models,'atmFModel');atmBars('atmByContract',d.contracts,'atmFContract');atmBars('atmByOwnership',d.ownership,'atmFOwnership');document.getElementById('atmRows').innerHTML=assets.map(x=>`<tr><td>${atmEsc(x.company)}</td><td>${atmEsc(x.line)}</td><td><b>${atmEsc(x.locality)}</b></td><td>${atmEsc(x.asset_key)}</td><td>${atmEsc(x.model)}</td><td>${atmEsc(x.contract)}</td><td>${atmEsc(x.ownership)}</td><td>${atmEsc(x.teamviewer||'—')}</td><td><code>${atmEsc(x.teamviewer_id||'—')}</code></td><td><code>${atmEsc(x.ip||'—')}</code></td><td>${atmEsc(x.status)}</td></tr>`).join('')||'<tr><td colspan="11">Sem ATMs no recorte.</td></tr>'}
if(document.getElementById('atmApply')){document.getElementById('atmApply').onclick=()=>loadAtmDashboard(false);document.getElementById('atmClear').onclick=()=>{['atmFCompany','atmFLine','atmFLocation','atmFModel','atmFContract','atmFOwnership','atmFStatus'].forEach(id=>document.getElementById(id).value='');loadAtmDashboard(false)};document.getElementById('atmDashRefresh').onclick=()=>loadAtmDashboard(false);document.getElementById('atmDashExport').onclick=()=>{if(!atmDashData)return;const rows=[['Operadora','Linha','Localidade','ATM','Modelo','Contrato','Posse','TeamViewer','ID TeamViewer','IP','Status'],...(atmDashData.assets||[]).map(x=>[x.company,x.line,x.locality,x.asset_key,x.model,x.contract,x.ownership,x.teamviewer,x.teamviewer_id,x.ip,x.status])];const csv=rows.map(r=>r.map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(';')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv'}));a.download='dashboard_atm.csv';a.click()};loadAtmDashboard(true)}
