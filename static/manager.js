...
['chipDashOperation','chipDashCompany','chipDashLine','chipDashLocation'].forEach(id=>document.getElementById(id)?.addEventListener('change',chipDashRender));document.getElementById('chipDashExport')?.addEventListener('click',()=>{const f=chipDashFilteredRows();const p=new URLSearchParams();if(f.op)p.set('operation',f.op);if(f.c)p.set('company',f.c);if(f.l)p.set('line',f.l);if(f.loc)p.set('location',f.loc);window.location.href='/api/chip-swaps/export.xlsx?'+p.toString()});loadChipDashboard();
