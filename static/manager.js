...
function dashPanRenderSummary(){
  ...
  const done=a.filter(x=>emvNormStatus(x.status)==='CONCLUIDA').length;
  const progress=a.filter(x=>emvNormStatus(x.status)==='EM ANDAMENTO').length;
  const pending=a.filter(x=>emvNormStatus(x.status)==='PENDENTE').length;
...
// V56-D — Dashboard EMV: uma única implementação, aderente ao contrato real {ok, rows[]}.
let emvDashRows=[];
