...
function chipDashFilteredRows(){const op=document.getElementById('chipDashOperation')?.value||'',c=document.getElementById('chipDashCompany')?.value||'',l=document.getElementById('chipDashLine')?.value||'',loc=document.getElementById('chipDashLocation')?.value||'',test=document.getElementById('chipDashTestResult')?.value||'';const base=(chipDashData.locations||[]).filter(x=>!op||chipDashOperation(x.company)===op);let rows=base.filter(x=>(!c||x.company===c)&&(!l||x.line===l)&&(!loc||x.location===loc));if(test)rows=rows.map(x=>({...x,validators:(x.validators||[]).filter(v=>(v.test_result||'SEM_RESULTADO')===test)})).filter(x=>x.validators.length);return {op,c,l,loc,test,base,rows}}
...
// V41.1 — Dashboard EMV: status e evolução temporal
(async()=>{const panel=document.querySelector('[data-v23-panel="emv"]');if(!panel)return;try{const j=await (await fetch('/api/emv-chip-swaps',{cache:'no-store'})).json(),allRows=j.rows||[],c=document.getElementById('execCompany')?.value||'',l=document.getElementById('execLine')?.value||'',st=document.getElementById('execStatus')?.value||'',rows=allRows.filter(x=>(!c||x.company===c)&&(!l||x.line===l)&&(!st||(st==='CONCLUIDA'?x.status==='CONCLUÍDA':st==='PENDENTE'?x.status==='PENDENTE':st==='EM ANDAMENTO'?x.status==='EM ANDAMENTO':true))),total=rows.length,done=rows.filter(x=>x.status==='CONCLUÍDA').length,pend=total-done,pct=total?Math.round(done/total*1000)/10:0;const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v};set('emvDashTotal',total);set('emvDashDone',done);set('emvDashPending',pend);set('emvDashPct',pct+'%');set('emvDashDonePct',pct+'%');set('emvDashText',`${done} de ${total} bloqueios concluídos.`);const bd=document.getElementById('emvBarDone'),bp=document.getElementById('emvBarPending');if(bd)bd.style.width=pct+'%';if(bp)bp.style.width=(100-pct)+'%';const days={};rows.filter(x=>x.completed_at).forEach(x=>{const d=x.completed_at.slice(0,10);days[d]=(days[d]||0)+1});let cumulative=0;const data=Object.entries(days).sort().map(([d,n])=>({d,n,c:(cumulative+=n)})),el=document.getElementById('emvEvolution');if(el){if(!data.length){el.innerHTML='<p class="muted">Ainda não há histórico de conclusões.</p>'}else{const max=Math.max(...data.map(x=>x.c),1);el.innerHTML=data.map(x=>`<div class="emvEvolutionRow"><span>${x.d.split('-').reverse().join('/')}</span><div><i style="width:${x.c/max*100}%"></i></div><b>${x.c}</b></div>`).join('')}}}catch(e){console.warn('EMV dashboard',e)}})();['execCompany','execLine','execStatus'].forEach(id=>document.getElementById(id)?.addEventListener('change',()=>setTimeout(()=>location.reload(),50)));
