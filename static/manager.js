...
function chipDashRender(){const c=document.getElementById('chipDashCompany')?.value||'',l=document.getElementById('chipDashLine')?.value||'',loc=document.getElementById('chipDashLocation')?.value||'';let rows=(chipDashData.locations||[]).filter(x=>(!c||x.company===c)&&(!l||x.line===l)&&(!loc||x.location===loc));chipDashFill('chipDashLine',(chipDashData.locations||[]).filter(x=>!c||x.company===c).map(x=>x.line),'Todas');chipDashFill('chipDashLocation',(chipDashData.locations||[]).filter(x=>(!c||x.company===c)&&(!l||x.line===l)).map(x=>x.location),'Todas');const total=rows.reduce((a,x)=>a+x.total,0),done=rows.reduce((a,x)=>a+x.concluded,0),prog=rows.reduce((a,x)=>a+x.in_progress,0),pend=rows.reduce((a,x)=>a+x.pending,0),pct=total?Math.round(done/total*1000)/10:0;[['chipDashTotal',total],['chipDashDone',done],['chipDashProgressN',prog],['chipDashPending',pend],['chipDashPct',pct+'%']].forEach(([i,v])=>{const e=document.getElementById(i);if(e)e.textContent=v});const bar=document.getElementById('chipDashBar');if(bar)bar.style.width=Math.min(pct,100)+'%';const el=document.getElementById('chipDashLocations');if(el)el.innerHTML=`<table class="chipDashTable"><thead><tr><th>Localidade</th><th>Progresso</th><th>Concl.</th><th>Pend.</th></tr></thead><tbody>${rows.sort((a,b)=>a.percent-b.percent).map(x=>`<tr><td>${chipDashEsc(x.line)} · ${chipDashEsc(x.location)}</td><td><b>${x.percent}%</b></td><td>${x.concluded}/${x.total}</td><td>${x.pending}</td></tr>`).join('')||'<tr><td colspan="4">Sem dados.</td></tr>'}</tbody></table>`;const tech={};rows.forEach(x=>(x.validators||[]).forEach(v=>{if(!v.technician)return;const t=tech[v.technician]??={name:v.technician,total:0,done:0};t.total++;if(v.status==='CONCLUÍDA')t.done++}));const te=document.getElementById('chipDashTechs');if(te)te.innerHTML=`<table class="chipDashTable"><thead><tr><th>Técnico</th><th>Concluídas</th><th>Atividades</th></tr></thead><tbody>${Object.values(tech).sort((a,b)=>b.done-a.done).map(t=>`<tr><td>${chipDashEsc(t.name)}</td><td><b>${t.done}</b></td><td>${t.total}</td></tr>`).join('')||'<tr><td colspan="3">Sem atividade registrada.</td></tr>'}</tbody></table>`}
async function loadChipDashboard(){try{const j=await fetch('/api/chip-swaps/dashboard',{cache:'no-store'}).then(r=>r.json());if(!j.ok)return;chipDashData=j;chipDashFill('chipDashCompany',j.locations.map(x=>x.company),'Todas');chipDashFill('chipDashLine',j.locations.map(x=>x.line),'Todas');chipDashFill('chipDashLocation',j.locations.map(x=>x.location),'Todas');chipDashRender()}catch(e){console.warn('chip dashboard',e)}}
['chipDashCompany','chipDashLine','chipDashLocation'].forEach(id=>document.getElementById(id)?.addEventListener('change',chipDashRender));loadChipDashboard();
