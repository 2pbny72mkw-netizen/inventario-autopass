...
// ===== V5.1 · Localização operacional da equipe =====
const AUTOPASS_TEAM_LOCATION_V5 = 'autopass-team-location-v5';
...
async function sendTeamPosition(position){
  const now=Date.now();
  if(now-teamLocationLastSent < 90000) return; // máximo ~1 envio/90s
  ...
  try{
    ...
    const a=Number.isFinite(position.coords.accuracy)?Math.round(position.coords.accuracy):null;
    setTeamLocationStatus(
      `Localização ativa · última atualização agora${a!==null?` · precisão ~${a} m`:''}.`,
      true
    );
  }catch(err){
    setTeamLocationStatus(`Localização ativa, mas não foi possível enviar: ${err.message}`);
  }
}
function stopTeamLocation(){
  ...
  localStorage.removeItem(AUTOPASS_TEAM_LOCATION_V5);
...
function startTeamLocation(){
  if(!('geolocation' in navigator)){
    setTeamLocationStatus('Geolocalização não disponível neste aparelho.');
    return;
  }
  ...
  teamLocationWatchId=navigator.geolocation.watchPosition(
    ...
    err=>{
      ...
      setTeamLocationStatus(messages[err.code]||'Não foi possível obter a localização.');
      if(err.code===1) stopTeamLocation();
    },
    {enableHighAccuracy:true,maximumAge:60000,timeout:15000}
  );
  localStorage.setItem(AUTOPASS_TEAM_LOCATION_V5,'1');
...
if($('teamLocationToggle')){
  ...
  if(localStorage.getItem(AUTOPASS_TEAM_LOCATION_V5)==='1'){
    startTeamLocation();
  }
}
