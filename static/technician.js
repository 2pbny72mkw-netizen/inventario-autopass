...
async function loadAlready() {
  ...
  const canManage = window.CURRENT_USER_ROLE === 'manager';
...
function pendingDefinitions() {
  if (!current) return [];
  return [
    { type: 'ATM', label: 'ATMs', expected: Number(current.expected_atm || 0) },
    { type: 'Validador de Recarga', label: 'Validadores de Recarga', expected: Number(current.expected_validator || 0) },
    { type: 'POS de Bilheteria', label: 'POS de Bilheteria', expected: Number(current.expected_pos || 0) }
  ].map(item => {
    const done = countInventoryByType(item.type);
    const detailed = allLocationAssets.filter(a => normalizedEquipmentType(a.equipment_type || 'ATM') === item.type).length;
    return {...item, done, detailed, remaining: Math.max(item.expected-done,0), extra: Math.max(done-item.expected,0)};
  });
}
...
function renderLocationPending() {
  ...
  summary.innerHTML=totalRemaining
    ...
    : 'Esta localidade não possui quantidade prevista de ATM, Validador ou POS na base.';
...
function updateEquipmentTypeUI(){
  ...
  const vf=$('validatorFields'); if(vf) vf.style.display=type==='Validador de Recarga'?'block':'none';
}
...
$('base_asset_id').onchange = () => {
  const a=assets.find(x=>x.id==+$('base_asset_id').value); if(!a) return;
  ...
  if($('mount')) $('mount').value=a.mount||'';
  if($('application')) $('application').value=a.application||'';
  if($('bom_id')) $('bom_id').value=a.bom_id||'';
  if($('bu_id')) $('bu_id').value=a.bu_id||'';
  if($('validator_top_id')) $('validator_top_id').value=a.top_id||'';
...
async function enqueueCurrentForm(form) {
  try{
    ...
    form.reset(); $('location_id').value=current.id; clearGpsFields(); updateEquipmentTypeUI();
...
$('invForm').onsubmit = async e => {
  ...
  if(editingInventoryId){
    try{
      ...
      editingInventoryId=null; e.target.reset(); $('location_id').value=current.id; clearGpsFields(); updateEquipmentTypeUI();
