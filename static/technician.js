...
const uniq = a =>
  [...new Set(a)].sort((x, y) => x.localeCompare(y, 'pt-BR'));
...
function applyGpsPosition(position) {
  ...
  const capturedAt =
    new Date(position.timestamp || Date.now()).toISOString();
...
function captureGpsForSubmission() {
  ...
  if (!('geolocation' in navigator)) {
    ...
    setGpsMessage(
      'GPS não disponível neste aparelho/navegador. O cadastro pode continuar.',
      false
    );
  ...
  return new Promise(resolve => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      error => {
        ...
        setGpsMessage(
          `${messages[error.code] ||
            'Não foi possível obter o GPS.'} O cadastro pode continuar.`,
          false
        );
...
const LOCAL_MODE_KEY =
  'inventario-autopass-local-mode';
    ...
    ordered.map(x => {
      ...
      return `
        ...
        ">
          <b>
            ${escapeHtml(
              x.fields.equipment_type ||
              'Equipamento'
            )}
            •
            ${escapeHtml(
              x.fields.asset_identifier ||
              ''
            )}
          </b>
          <br>
...
async function loadLocations() {
  ...
  if (navigator.onLine) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        throw new Error(
          'Falha ao consultar localidades.'
        );
      }
  ...
  if (!locations.length) {
    ...
    showMsg(
      'Sem internet e sem localidades armazenadas neste aparelho. Conecte-se uma vez antes de entrar em campo.',
      false
    );
  }
}
...
function hideInfo() {
  ...
  $('already').innerHTML =
    '<tr><td colspan="6">Selecione um local.</td></tr>';
...
function showInfo() {
  ...
  const expected =
    Number(current.expected_atm || 0) +
    Number(current.expected_validator || 0) +
    Number(current.expected_pos || 0);
  ...
  $('locInfo').innerHTML = `
    ...
    • ${escapeHtml(current.company)}
    • ${escapeHtml(current.line)}
    <br>
    ...
    Base:
    ATM ${current.expected_atm || 0}
    |
    Validadores ${current.expected_validator || 0}
    |
    POS ${current.expected_pos || 0}
    |
    Total ${expected}
  `;
}
...
async function queuedForLocation(
  locationId
) {
...
async function loadAlready() {
  ...
  if (navigator.onLine) {
    ...
    try {
      ...
      const r =
        await fetch(
          `/api/location/${current.id}/inventory`,
          { cache: 'no-store' }
        );
              ...
              x._pending
                ...
                : `
                  ...
                  ">
                    <button
                      type="button"
                      class="secondary editInventoryBtn"
                      data-id="${x.id}"
                    >
                      Editar
                    </button>
                    ...
                    <button
                      type="button"
                      class="secondary deleteInventoryBtn"
                      data-id="${x.id}"
                      style="
                        color:#b42318;
                        border-color:#f0b4b4;
...
function renderAssets() {
  ...
  sel.innerHTML =
    '<option value="">Novo / não selecionar</option>';
  ...
  $('assetHint').textContent =
    assets.length
      ...
      ? `${
          assets.filter(
            x =>
              x.already_inventoried
          ).length
        } já feitos de ${
          assets.length
        } ATM(s) encontrados na base detalhada.`
...
/* =========================================================
   PENDÊNCIAS OPERACIONAIS DA LOCALIDADE
========================================================= */

function normalizedEquipmentType(
  value
) {
...
function renderLocationPending() {
  ...
  if (!current) {
    ...
    summary.textContent =
      'Selecione uma localidade para consultar o que falta.';
  ...
  summary.innerHTML =
    totalRemaining
      ...
      ? `
        ...
        equipamento(s) previstos conferidos.
        Ainda faltam
        <b>
          ${totalRemaining}
        </b>.
      ...
      : totalExpected
        ...
        ? `
          <b>
            Previsão da base atendida.
          </b>
          ${totalDone}
          de
          ${totalExpected}
          equipamento(s) conferidos.
        `

        : `
          Esta localidade não possui quantidade
          prevista de ATM, Validador ou POS
          na base.
        `;
      ...
      const action =
        x.remaining > 0
          ...
          ? `
            <button
              type="button"
              class="secondary pendingStartBtn"
              data-type="${escapeHtml(x.type)}"
            >
              Cadastrar ${escapeHtml(x.type)}
            </button>
          `
  ...
  ?.addEventListener(
    ...
    () => {
      ...
      if (!current) {
        ...
        showMsg(
          'Selecione uma localidade para consultar as pendências.',
          false
        );
...
/* =========================================================
   CONVERSÃO PARA OFFLINE
========================================================= */

async function formToOfflineRecord(
  form
) {
...
async function hasLocalDuplicate(
  record
) {
...
async function sendRecord(
  ...
  if (
    ...
    return {
      ...
      message:
        j.error ||
        'Duplicidade já existente no servidor.'
    };
  }
...
async function syncQueue({ silent = false } = {}) {
  ...
  if (!navigator.onLine) {
    ...
    if (!silent) {
      ...
      showMsg(
        'Sem conexão. A sincronização será tentada quando o sinal voltar.',
        false
      );
    }
  ...
  if (!silent) {
    ...
    if (!remaining) {
      ...
      showMsg(
        `${synced} registro(s) sincronizado(s). ${remaining} ainda aguardam envio.`,
        false
      );
    }
  }
...
$('invForm').onsubmit =
  async e => {
    ...
    /*
      Modo coleta local:
      sempre guarda no aparelho,
      mesmo que exista internet.
    */

    if (isLocalMode()) {

      await enqueueCurrentForm(
        e.target
      );
    ...
    */
    ...
    try {
      ...
      showMsg(
        'Dados salvos com sucesso. O formulário foi limpo e as pendências da localidade foram atualizadas.',
        true
      );
      ...
      /*
        Limpa somente os campos do equipamento.
        Empresa, linha e localidade permanecem selecionadas.
      */
      ...
      /*
        Atualiza realizados e pendências
        da estação atual.
      */
      ...
      /*
        Se houver falha inesperada
        de rede/servidor, salva localmente.
      */
...
$('completeBtn').onclick =
  async () => {
    ...
    /*
      Primeiro verifica registros
      ainda não sincronizados neste aparelho.
    */
    ...
    if (!navigator.onLine) {
      ...
      return showMsg(
        'A conclusão da localidade exige conexão para evitar encerramento sem sincronizar os registros.',
        false
      );
    }

    /*
      Confere também as pendências
      operacionais esperadas pela base.
    */
    ...
    if (
      ...
      const proceed =
        confirm(
          `A base ainda indica ${operationalPending} equipamento(s) pendente(s) nesta localidade. Deseja mesmo assim continuar para a confirmação final?`
        );
    ...
    const confirmComplete =
      confirm(
        'Confirma que o levantamento desta localidade foi finalizado? Ela aparecerá como CONCLUÍDA no painel gerencial.'
      );
    ...
    const r =
      await fetch(
        `/api/location/${current.id}/complete`,
        {
          method: 'POST'
        }
      );
    ...
    if (r.ok) {
      ...
      showMsg(
        'Localidade marcada como CONCLUÍDA.',
        true
      );
      ...
      showMsg(
        'Não foi possível concluir a localidade.',
        false
      );
    }
  };
...
window.addEventListener(
  ...
  async () => {
    ...
    if (
      ...
      showMsg(
        `Conexão restabelecida. ${queue.length} registro(s) aguardam envio. Clique em "Sincronizar agora" quando desejar.`,
        true
      );
    }
  }
  ...
  .addEventListener(
    ...
    async e => {
      ...
      if (
        ...
        showMsg(
          'Modo coleta local ativado. Os próximos registros ficarão neste aparelho até a sincronização manual.',
          true
        );
        ...
        if (
          ...
          showMsg(
            `${queue.length} registro(s) continuam pendentes. Clique em "Sincronizar agora" quando desejar enviá-los.`,
            true
          );
...
(async function init() {
  ...
  try {
    ...
    showMsg(
      'Não foi possível iniciar o armazenamento offline neste navegador.',
      false
    );
  }
