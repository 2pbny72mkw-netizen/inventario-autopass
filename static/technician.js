  ...
  $('pendingList').innerHTML = ordered.map(x => {
    ...
    return `
      <div style="padding:10px 0;border-bottom:1px solid #e5e7eb">
        ...
        <small>${escapeHtml(x.location_name || '')} • ${dt} • ${files} mídia(s)</small>
      </div>`;
  }).join('');
}
...
async function enqueueCurrentForm(form) {
  ...
  showMsg('Sem conexão: equipamento salvo neste aparelho e aguardando sincronização.', true);
...
$('invForm').onsubmit = async e => {
  ...
  try {
    ...
    if (!r.ok) {
      // Se a rede caiu durante o envio, guardamos localmente.
...
(async function init() {
  try {
    ...
    // Ao abrir a tela com internet, tenta esvaziar pendências antigas.
    if (navigator.onLine) {
      await syncQueue({ silent: true });
    }
  } catch (err) {
