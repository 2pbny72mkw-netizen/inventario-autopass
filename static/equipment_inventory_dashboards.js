...
function vals(el){return el.multiple?[...el.options].filter(o=>o.selected).map(o=>o.value).filter(Boolean):(el.value?[el.value]:[])}function params(panel){const q=new URLSearchParams();panel.querySelectorAll("select[data-filter]").forEach(el=>vals(el).forEach(v=>q.append(el.dataset.filter,v)));return q}
function multiLabel(el){const v=vals(el),b=el._multiButton;if(!b)return;b.textContent=v.length===0?'Todos':v.length===1?v[0]:`${v.length} selecionados`;b.title=v.length?v.join(' + '):'Todos'}
function renderMulti(el){const box=el._multiBox;if(!box)return;const selected=new Set(vals(el));box.innerHTML='';[...el.options].forEach(o=>{const lab=document.createElement('label');lab.className='invMultiOption';const cb=document.createElement('input');cb.type='checkbox';cb.checked=selected.has(o.value);cb.value=o.value;const sp=document.createElement('span');sp.textContent=o.textContent;lab.append(cb,sp);cb.addEventListener('change',()=>{o.selected=cb.checked;multiLabel(el);el.dispatchEvent(new Event('change',{bubbles:true}))});box.appendChild(lab)});multiLabel(el)}
function fillSelect(el,values,keep){const cur=Array.isArray(keep)?keep:vals(el);const all=[...new Set([...(values||[]),...cur])];el.innerHTML=all.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join("");[...el.options].forEach(o=>o.selected=cur.includes(o.value));renderMulti(el)}
function enhanceMulti(el){if(el.dataset.multiReady)return;el.dataset.multiReady='1';el.multiple=true;const wrap=document.createElement('div');wrap.className='invMulti';const btn=document.createElement('button');btn.type='button';btn.className='invMultiButton';const box=document.createElement('div');box.className='invMultiMenu';wrap.append(btn,box);el.parentNode.insertBefore(wrap,el);wrap.appendChild(el);el.classList.add('invMultiNative');el._multiButton=btn;el._multiBox=box;btn.addEventListener('click',e=>{e.stopPropagation();document.querySelectorAll('.invMulti.open').forEach(x=>{if(x!==wrap)x.classList.remove('open')});wrap.classList.toggle('open')});box.addEventListener('click',e=>e.stopPropagation());document.addEventListener('click',()=>wrap.classList.remove('open'));renderMulti(el)}
...
async function load(panel){
 ...
 try{
   ...
   [["company","companies"],["line","lines"],["locality","localities"],["model","models"],["status","statuses"],["subtype","subtypes"]].forEach(([f,o])=>{const el=panel.querySelector(`[data-filter="${f}"]`);if(el)fillSelect(el,opts[o]||[],vals(el))});
   bars(panel.querySelector('[data-chart="companies"]'),d.companies,12,k=>{const e=panel.querySelector('[data-filter="company"]');if(e){[...e.options].forEach(o=>{if(o.value===k)o.selected=!o.selected});renderMulti(e);load(panel)}});
   const locChart=panel.querySelector('[data-chart="locations"]');if(locChart)bars(locChart,d.locations,12,k=>{const e=panel.querySelector('[data-filter="locality"]');if(e){[...e.options].forEach(o=>{if(o.value===k)o.selected=!o.selected});renderMulti(e);load(panel)}});
   ...
   if(stationBody){
     ...
     stationBody.querySelectorAll("tr[data-station]").forEach(tr=>tr.addEventListener("click",()=>{const e=panel.querySelector('[data-filter="locality"]');if(e){const o=[...e.options].find(o=>o.value===tr.dataset.station);if(o){o.selected=true;renderMulti(e)}load(panel)}}));
   }
   bars(panel.querySelector('[data-chart="statuses"]'),d.statuses,10,k=>{const e=panel.querySelector('[data-filter="status"]');if(e){[...e.options].forEach(o=>{if(o.value===k)o.selected=!o.selected});renderMulti(e);load(panel)}});
   ...
   const sub=panel.querySelector('[data-chart="subtypes"]'); if(sub) bars(sub,d.subtypes,6,k=>{const e=panel.querySelector('[data-filter="subtype"]');if(e){[...e.options].forEach(o=>{if(o.value===k)o.selected=!o.selected});renderMulti(e);load(panel)}});
...
function init(panel){
 ...
 panel.querySelectorAll("[data-filter]").forEach(enhanceMulti);
 panel.querySelectorAll("[data-filter]").forEach(el=>el.addEventListener("change",()=>load(panel)));
 panel.querySelector(".invClear")?.addEventListener("click",()=>{panel.querySelectorAll("select[data-filter]").forEach(e=>{[...e.options].forEach(o=>o.selected=false);renderMulti(e)});load(panel)});
