...
(() => {
  ...
  function enableDrillMove(){const card=$('dashDrillCard'),handle=$('dashDrillHandle');if(!card||!handle||card.dataset.dragReady)return;card.dataset.dragReady='1';let drag=false,sx=0,sy=0,sl=0,st=0;handle.addEventListener('mousedown',e=>{if(e.target.closest('button'))return;const r=card.getBoundingClientRect();card.style.position='fixed';card.style.left=r.left+'px';card.style.top=r.top+'px';card.style.margin='0';sx=e.clientX;sy=e.clientY;sl=r.left;st=r.top;drag=true;e.preventDefault()});window.addEventListener('mousemove',e=>{if(!drag||card.classList.contains('dashDrillMax'))return;card.style.left=Math.min(innerWidth-120,Math.max(0,sl+e.clientX-sx))+'px';card.style.top=Math.min(innerHeight-80,Math.max(72,st+e.clientY-sy))+'px'});window.addEventListener('mouseup',()=>drag=false);$('dashDrillMax')?.addEventListener('click',()=>{card.classList.toggle('dashDrillMax');$('dashDrillMax').textContent=card.classList.contains('dashDrillMax')?'Restaurar':'Maximizar';if(!card.classList.contains('dashDrillMax')){card.style.left='';card.style.top='';card.style.position='';}})}
  function openDrill(title,rows){drillTitle=title;drillRows=rows||[];setText('dashDrillTitle',title);const m=$('dashDrillModal');if(m){m.hidden=false;document.body.classList.add('dashDrillOpen');enableDrillMove();}if($('dashDrillSearch'))$('dashDrillSearch').value='';renderDrillRows()}
