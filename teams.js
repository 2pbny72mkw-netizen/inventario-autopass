window.AUTOPASS_TEAMS_VERSION='teams-v9-3';
console.log('AUTOPASS Central Operacional V9.3 carregada');
...
function avatarHtml(t){
  ...
  if(t.photo_url){
    ...
    const versionedPhoto=`${t.photo_url}${String(t.photo_url).includes('?')?'&':'?'}v=${encodeURIComponent(t.photo_version||t.captured_at||'v93')}`;
    const safeUrl=String(versionedPhoto).replace(/[\"'()]/g, encodeURIComponent);
...
function markerIcon(t){
  ...
  const photo=t.photo_url
    ? `<span class="teamMapAvatarPhoto" style="background-image:url(&quot;${esc(String(`${t.photo_url}${String(t.photo_url).includes('?')?'&':'?'}v=${encodeURIComponent(t.photo_version||t.captured_at||'v93')}`).replace(/[\"'()]/g, encodeURIComponent))}&quot;)"></span>`
