...
function chipDashOperation(company){const t=String(company||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase();if(t.includes('CPTM'))return 'CPTM';if(t.includes('METRO')||t.includes('MÊTRO'))return 'Metrô';return company||'Outros'}
...
['chipDashOperation','chipDashCompany','chipDashLine','chipDashLocation'].forEach(id=>document.getElementById(id)?.addEventListener('change',chipDashRender));loadChipDashboard();
