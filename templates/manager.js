...
// V77.8 — Visões panorâmicas: frontend alinhado à API /api/panoramas.
let dashPanData=[];
function v778PanFiltered(){const c=document.getElementById('dashPanCompany')?.value||'',l=document.getElementById('dashPanLine')?.value||'',st=document.getElementById('dashPanStatus')?.value||'';return dashPanData.filter(x=>(!c||x.company===c)&&(!l||x.line===l)&&(!st||x.status===st))}
function v778PanMetrics(){const a=v778PanFiltered(),total=a.length,done=a.filter(x=>x.status==='CONCLUÍDA').length,prog=a.filter(x=>x.status==='EM ANDAMENTO').length,pend=a.filter(x=>x.status==='PENDENTE').length,photos=a.reduce((n,x)=>n+(+x.photo_count||0),0),pct=total?Math.round(done/total*100):0;const put=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v};put('dashPanTotal',total);put('dashPanPending',pend);put('dashPanInProgress',prog);put('dashPanDone',done);put('dashPanPhotos',photos);put('dashPanPendingPct',total?Math.round(pend/total*100)+'%':'0%');put('dashPanInProgressPct',total?Math.round(prog/total*100)+'%':'0%');put('dashPanDonePct',pct+'%');put('dashPanPct',pct+'%');put('dashPanLegendDone',done);put('dashPanLegendProgress',prog);put('dashPanLegendPending',pend);put('dashPanProgressText',`${done} de ${total} localidades concluídas.`);const donut=document.getElementById('dashPanDonut');if(donut){const d=total?done/total*100:0,p=total?prog/total*100:0;donut.style.background=`conic-gradient(#1aa36f 0 ${d}%,#e59a18 ${d}% ${d+p}%,#d94747 ${d+p}% 100%)`}const bars=[['dashPanBarDone',done],['dashPanBarProgress',prog],['dashPanBarPending',pend]];bars.forEach(([id,n])=>{const e=document.getElementById(id);if(e)e.style.width=(total?n/total*100:0)+'%'});return a}
function v396PanLocations(){const c=document.getElementById('dashPanCompany')?.value||'',l=document.getElementById('dashPanLine')?.value||'',el=document.getElementById('dashPanLocation');const a=v778PanMetrics();if(!el)return;const cur=el.value;el.innerHTML='<option value="">Selecione</option>'+a.map(x=>`<option value="${x.id}">${esc(x.location)} · ${esc(x.status)}</option>`).join('');if([...el.options].some(o=>o.value===cur))el.value=cur;const g=document.getElementById('dashPanGallery');if(g&&!el.value)g.innerHTML='<p class="muted">Selecione uma localidade.</p>'}
function v396ShowPan(){const id=+document.getElementById('dashPanLocation')?.value,x=dashPanData.find(a=>a.id===id),g=document.getElementById('dashPanGallery');if(!g)return;if(!x){g.innerHTML='<p class="muted">Selecione uma localidade.</p>';return}g.innerHTML=(x.points||[]).flatMap(p=>(p.photos||[]).map(ph=>`<figure><a href="${esc(ph.url)}" target="_blank"><img loading="lazy" src="${esc(ph.thumb_url||ph.url)}"></a><figcaption><b>${esc(p.name)}</b><br>${esc(ph.uploaded_by)} · ${ph.created_at?new Date(ph.created_at).toLocaleString('pt-BR'):'-'}</figcaption></figure>`)).join('')||'<p class="muted">Nenhuma foto nesta localidade.</p>'}
async function v396LoadPanorama(){try{const r=await fetch('/api/panoramas',{cache:'no-store'}),d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'Falha');dashPanData=d.locations||[];const fill=(id,vals,label)=>{const e=document.getElementById(id);if(!e)return;const cur=e.value;e.innerHTML=`<option value="">${label}</option>`+[...new Set(vals.filter(Boolean))].sort().map(v=>`<option>${esc(v)}</option>`).join('');if([...e.options].some(o=>o.value===cur))e.value=cur};fill('dashPanCompany',dashPanData.map(x=>x.company),'Todas');fill('dashPanLine',dashPanData.map(x=>x.line),'Todas');v396PanLocations()}catch(e){console.warn('Visão Panorâmica',e)}}
document.getElementById('dashPanCompany')?.addEventListener('change',()=>{const c=document.getElementById('dashPanCompany').value;const line=document.getElementById('dashPanLine');if(line){const vals=dashPanData.filter(x=>!c||x.company===c).map(x=>x.line);line.innerHTML='<option value="">Todas</option>'+[...new Set(vals.filter(Boolean))].sort().map(v=>`<option>${esc(v)}</option>`).join('')}v396PanLocations()});document.getElementById('dashPanLine')?.addEventListener('change',v396PanLocations);document.getElementById('dashPanStatus')?.addEventListener('change',v396PanLocations);document.getElementById('dashPanLocation')?.addEventListener('change',v396ShowPan);v396LoadPanorama();
...
function chipDashFilteredRows(){const op=document.getElementById('chipDashOperation')?.value||'',c=document.getElementById('chipDashCompany')?.value||'',l=document.getElementById('chipDashLine')?.value||'',loc=document.getElementById('chipDashLocation')?.value||'',status=document.getElementById('chipDashStatus')?.value||'';const base=(chipDashData.locations||[]).filter(x=>!op||chipDashOperation(x.company)===op);let rows=base.filter(x=>(!c||x.company===c)&&(!l||x.line===l)&&(!loc||x.location===loc));if(status)rows=rows.map(x=>{const validators=(x.validators||[]).filter(v=>(v.status||'PENDENTE')===status);const concluded=validators.filter(v=>v.status==='CONCLUÍDA').length,in_progress=validators.filter(v=>v.status==='EM ANDAMENTO').length,pending=validators.length-concluded-in_progress;return {...x,validators,total:validators.length,concluded,in_progress,pending,percent:validators.length?Math.round(concluded/validators.length*1000)/10:0}}).filter(x=>x.total);return {op,c,l,loc,status,base,rows}}
...
['chipDashOperation','chipDashCompany','chipDashLine','chipDashLocation'].forEach(id=>document.getElementById(id)?.addEventListener('change',chipDashRender));document.getElementById('chipDashExport')?.addEventListener('click',chipDashExportCsv);loadChipDashboard();
